...
Uses print-based logging with level prefixes for visibility.
...
class Logger:
    """Simple logger with level prefixes and timestamps.

    Provides consistent logging across the application using print()
    statements with level indicators and timestamps.
    """
    ...
    def _format_message(self, level_prefix: str, level: str, message: str) -> str:
        """Format log message with timestamp, level prefix, and optional name.

        Args:
            level_prefix: Level prefix (e.g., "[INFO]", "[ERROR]")
            level: Log level
            message: Log message
        ...
        return f'{level_prefix} [{timestamp}] {prefix} {message}'
    ...
    def debug(self, message: str):
        ...
        if self._should_log('DEBUG'):
            formatted = self._format_message('[DEBUG]', 'DEBUG', message)
            print(formatted)
    ...
    def info(self, message: str):
        ...
        if self._should_log('INFO'):
            formatted = self._format_message('[INFO]', 'INFO', message)
            print(formatted)
    ...
    def success(self, message: str):
        ...
        """
        if self._should_log('INFO'):
            formatted = self._format_message('[SUCCESS]', 'SUCCESS', message)
            print(formatted)
    ...
    def warning(self, message: str):
        ...
        if self._should_log('WARNING'):
            formatted = self._format_message('[WARNING]', 'WARNING', message)
            print(formatted)
    ...
    def error(self, message: str, exc: Optional[Exception] = None):
        ...
        if self._should_log('ERROR'):
            if exc:
                formatted = self._format_message('[ERROR]', 'ERROR', f'{message}: {exc}')
            else:
                formatted = self._format_message('[ERROR]', 'ERROR', message)
            print(formatted)
    ...
    def critical(self, message: str, exc: Optional[Exception] = None):
        ...
        if self._should_log('CRITICAL'):
            if exc:
                formatted = self._format_message('[CRITICAL]', 'CRITICAL', f'{message}: {exc}')
            else:
                formatted = self._format_message('[CRITICAL]', 'CRITICAL', message)
            print(formatted)
